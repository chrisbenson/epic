...
func createPostgreSQLUserAndDatabase(adminUser string, adminPassword string, server string, database string, epicPassword string) error {
	...
	if err != nil {
		return errors.Wrap(err, "Error in db.Prepare() for createRoleQuery()")
	}
	defer stmt.Close()
	_, err = stmt.Exec()
	if err != nil {
	...
	stmt, err = db.Prepare(createDatabaseQuery(database))
	if err != nil {
	...
	stmt, err = db.Prepare(grantDbPrivQuery(database))
	if err != nil {
...
func createDatabaseQuery(database string) string {
	return "CREATE DATABASE " + database + " OWNER epic;"
}

func grantDbPrivQuery(database string) string {
	return "GRANT ALL PRIVILEGES ON DATABASE " + database + " to epic;"
}
...
func createPostgreSQLSchemaTablesAndConstraints(epicPassword string, server string, database string) error {

	var err error
	//conn := "postgres://" + adminUser + ":" + adminPassword + "@" + server + "/epic?sslmode=disable"
	conn := "postgres://epic:" + epicPassword + "@" + server + "/" + database + "?sslmode=disable"
...
func createPostgreSQLEpicAppAdminUser(epicPassword string, server string, database string) (string, error) {
...
func dropPostgreSQLSchemaTablesAndConstraints(adminUser string, adminPassword string, server string, database string) error {

	var err error
	conn := "postgres://" + adminUser + ":" + adminPassword + "@" + server + "/" + database + "?sslmode=disable"
...
func dropPostgreSQLUserAndDatabase(adminUser string, adminPassword string, server string, database string) error {
	...
	stmt, err := db.Prepare(dropDatabaseQuery(database))
	if err != nil {
...
func dropDatabaseQuery(database string) string {
