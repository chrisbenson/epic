...
func AssignTagToContentHandler(w http.ResponseWriter, r *http.Request) {
	...
	if AppID == "" {
		...
		if err != nil {
			http.Error(w, errors.Wrap(err, "AssignTagToContentHandler() | Error getting AppID from fromContext()").Error(), http.StatusUnauthorized)
			return
		}
		var ok bool
		AppID, ok = appIDi.(string)
		if !ok {
			http.Error(w, errors.New("AssignTagToContentHandler() | AppID type assertion from interface{} to string failed.").Error(), http.StatusInternalServerError)
			return
		}
	}
...
func AboutHandler(w http.ResponseWriter, r *http.Request) {
	about := About{Name: "Epic CMS", Version: "0.0.3", Website: "https://github.com/chrisbenson/epic"}
	w.WriteHeader(http.StatusOK)
